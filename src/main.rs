...
//main parses a couple options from the CLI and sends it to "helpers" that are effectively programs in themselves.
// if user wants to list torrents - we use the
fn main() {
    let cliargs = rtorrent - remote::clistruct::Cli::from_args();
    ...
    // else if user passes some -t or torrents we need to parse those and do some action
        ...
        if cliargs.torrent.as_ref().unwrap().len() > 0 {
            for torr in cliargs.torrent.unwrap().iter().into_iter() {
                ...
                torrentHelper(
                    torr,
                    cliargs.rtorrenturl.clone(),
                    cliargs.tempdir.clone(),
                    cliargs.incompletedir.clone(),
                    cliargs.files,
                    cliargs.infobool,
                    cliargs.infofilebool,
                    cliargs.infopieces,
                    cliargs.infotracker,
                    cliargs.labels.clone(),
                    cliargs.movepath.clone(),
                    cliargs.findpath.clone(),
                    cliargs.tracker.clone(),
                    cliargs.trackerid.clone(),
                    cliargs.stop,
                    cliargs.start,
                    cliargs.verify,
                    cliargs.remove,
                    cliargs.removeAndDelete,
                );
            }
        } else {
        ...
        rtorrenthelper(
            &cliargs.rtorrenturl,
            cliargs.incompletedir,
            cliargs.incompletedirbool,
            cliargs.cachesize,
            cliargs.exitrtorrent,
            cliargs.sessioninfo,
            cliargs.sessionstats,
            cliargs.utp,
            cliargs.noutp,
        );
    }
...
pub fn torrentHelper(
    ...
    let onlyAlphanumericRtorrentURL: String = rtorrenturl
        .to_string()
        .chars()
    ...
    match torrent.parse::<i16>() {
        Ok(ok) => {
            ...
            let hashmap = tempfile::deserCompare::returnDeserializedHashMap(
                tempfile::previousRtorrentRemoteJSONS(
                    tempdir.clone(),
                    &onlyAlphanumericRtorrentURL,
                ),
            );
            let torInt = ok - 1;
